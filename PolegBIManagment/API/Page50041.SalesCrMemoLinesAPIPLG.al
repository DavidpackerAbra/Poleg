...
page 50341 "Sales Cr.Memo Lines API PLG"
{
    APIGroup = 'bi';
    APIPublisher = 'poleg';
    APIVersion = 'v2.0';
    ApplicationArea = All;
    Caption = 'salesCreditMemoLinesAPI';
    DelayedInsert = true;
    ...
    begin
        // Get Sales Credit Memo Header information
        if SalesCrMemoHeader.Get(Rec."Document No.") then begin
            InvoiceDateTxt := SalesCrMemoHeader."VAT Reporting Date";
            if InvoiceDateTxt = 0D then
                InvoiceDateTxt := SalesCrMemoHeader."Posting Date";

            CustomerNoTxt := SalesCrMemoHeader."Sell-to Customer No.";
            CustomerNameTxt := SalesCrMemoHeader."Sell-to Customer Name";
            CustomerOrderNoTxt := SalesCrMemoHeader."Return Order No.";
            OrderDateTxt := SalesCrMemoHeader."Document Date";

            // Currency handling
            if SalesCrMemoHeader."Currency Code" = '' then
                CurrencyTxt := 'ILS'
            else
                CurrencyTxt := SalesCrMemoHeader."Currency Code";
        end;

        // Calculate line amounts (negative for credit memo)
        ...
        // Delivery No. from return receipt
        DeliveryNoTxt := Rec."Return Receipt No.";
